...
/* Superficie para armazenar os desenhos */
...
//   painelImagem = gtk_image_new_from_file ("imagem.ppm");
    // exibe dialog de sucesso na operacao
...
// FUN��O PARA ABRIR O DIALOG SELETOR DE CORES
void selecionarCor () {
  ...
  // pegando o resultado da sele��o se clicado em OK
...
void desenharPoligono(){
    /* Cairo é uma bilioteca utilizada para criar vertores gráficos em 2D */
    cr = cairo_create (surface); // ponteiro da bilbioteca Cairo que cria a superfície de desenho
    cairo_set_line_width (cr, larguraLinha); // função da biblioteca cairo que recebe um ponteiro da superfície de desenho e a largura da linha de desenho
    cairo_set_source_rgb (cr,corPincel.r,corPincel.g,corPincel.b); // função da biblioteca cairo que recebe um ponteiro da superfície de desenho e a cor RGB da linha de desenho

    /*Laço de repetição para armazenar dados nos vetores tipo Cairo*/
    int ite; 
    ...
    /*Função que liga a coordenada final com a coodenada inicial*/
    cairo_close_path (cr);
    
    //cairo_line_to (cr,cordenadaX+50,cordenadaY);

    /*Essa função desaloca a memória utilizada ponteiro tipo Cairo, "apagando-o", permite que função esteja pronta para ser executada novamente*/
    cairo_stroke (cr);
